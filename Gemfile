# frozen_string_literal: true

source "https://rubygems.org"

# Priority
group :jekyll_plugins do 
  gem "jekyll-theme-chirpy", "~> 7.4", ">= 7.4.1"  # Github Compatible Versions
  gem "jekyll-compose"                            #Create new posts, drafts, and pages
  gem "jekyll-feed"                               #Github supported
  gem "jekyll-seo-tag"                            #Github supported
  gem "jekyll-sitemap"                            #Github supported
  #gem "github-pages"                              #not sure if this is the official way to test whether the site will run or not. 
end

# Testing
group :test do
  gem "html-proofer", "~> 5.0"  # validate HTML output
end


# gem "jekyll-ai-domain-data" #AI Domain Data
# this is causing some issue in reinvoking Regenerating .well-known/domain-profile.json